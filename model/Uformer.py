...
class PEFNs(nn.Module):
    def __init__(self, dim=32, hidden_dim=128, act_layer=nn.GELU,drop = 0., use_eca=False):
        ...
        self.dwconv = nn.Sequential(
            ...
            Shift8(groups=hidden_dim, stride=1),  # num_feat的值在SCNet中为64，可修改，可以是num_feat=dim  dim // 8
            act_layer())
        # self.dwconv = nn.Sequential(Shift8(groups=hidden_dim, stride=1))  # num_feat的值在SCNet中为64，可修改，可以是num_feat=dim  dim //
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return sattn
######################################################
#################ChannelAttentionSCIA模块###############
class ChannelAttentionSCIA(nn.Module):
    def __init__(self, dim, reduction = 8):
        super(ChannelAttentionSCIA, self).__init__()
...
class SCIA(nn.Module):
    def __init__(self, dim,  reduction=8):
        super(SCIA, self).__init__()
        self.sa = SpatialAttention()
        self.ca = ChannelAttentionSCIA(dim, reduction)
...
##########################################
########### SDETBlock #############
class SDETBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,token_projection='linear',token_mlp='pefns',token_attn='SCIA',
        ...
        if token_attn == 'CSA':
            self.ATTN = ChannelAttention(dim, num_heads, bias=qkv_bias)
        elif token_attn == 'SCIA':
            self.ATTN = SCIA(dim)
        else:
            raise Exception("ATTN error!")
        #############PEFNs模块###############
        self.norm3 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        if token_mlp in ['ffn','mlp']:
            self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim,act_layer=act_layer, drop=drop) 
        elif token_mlp=='pefns':
            self.mlp =  PEFNs(dim,mlp_hidden_dim,act_layer=act_layer, drop=drop)
        elif token_mlp=='pefn':
            self.mlp =  PEFN(dim,mlp_hidden_dim,act_layer=act_layer, drop=drop)
        else:
...
class BasicUformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
                 ...
                 token_projection='linear',token_mlp='pefns', token_attn='SCIA', shift_flag=True,
                 modulator=False,cross_modulator=False):
        ...
        if shift_flag:
            self.blocks = nn.ModuleList([
                SDETBlock(dim=dim, input_resolution=input_resolution,
                                    ...
                                    token_attn=token_attn, modulator=modulator,cross_modulator=cross_modulator)
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 ...
                 use_checkpoint=False, token_projection='linear', token_mlp='pefns',
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size=input_size, embed_dim=16,depths=depths,
                 win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='pefns', modulator=True, shift_flag=False)
