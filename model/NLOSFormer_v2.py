...
## NLOSFormer_v2：使用cross-attention的版本
...
class CrossAttention(nn.Module):
    def __init__(self, heads=1, dim_head=64):
        ...
        self.to_q = nn.Linear(query_dim, inner_dim, bias=False) # 输入三维数据，只作用在最后一维
...
class PSFEstimator(nn.Module):
    def __init__(self, nf):
        ...
        self.fusion = CrossAttention(heads=nf,dim_head=nf)
        self.body = nn.Sequential(
...
class NLOSFormer(nn.Module):
    def __init__(self, input_channel, hidden_channel=64, psf_dim=42,block_num=16):
        super().__init__()
        ...
        self.PSFEstimator = PSFEstimator(hidden_channel)
        self.Reconstructor = Reconstructor(hidden_channel, block_num)
    ...
    def forward(self, img):
        ...
        fpsf = self.psf_head(self.init_kernel).repeat(b, 1, 1, 1) # [b,c,1,1]
        
        # the residual connection is important
        fpsf = self.PSFEstimator(fimg, fpsf) + fpsf  # [b,c,1,1]
